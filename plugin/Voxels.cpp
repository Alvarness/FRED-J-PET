...
int Voxels::read(string path){
	size_t pos;
	int ierr = read_mhd_header(path,dtype,nn,x0,L,left,up,front,pos);
	if (!ierr){
		ierr = read_mhd_data(path,dtype,nn,pos,&data);
	}
	if (!ierr){
		L2hs();
		_N = 1UL*nn[0]*nn[1]*nn[2];
...
int Voxels::write(string path){
	return write_mhd(path,dtype,data,nn,x0,L,left,up,front);
}
