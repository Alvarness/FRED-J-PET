...
int write_mhd(string fname,string &dtype,char *data,int nn[3],float x0[3],float L[3],float left[3],float up[3],float front[3]);

template<typename T>
void write_mhd(string fname,vector<T> &A,int nn[3],float x0[3],float L[3],float left[3],float up[3],float front[3]){
	string dtype = dataTypeString<T>();
	write_mhd(fname,dtype,(char *)A.data(),nn,x0,L,left,up,front);
}
...
int read_mhd_header(string fname,string &dtype,int nn[3],float x0[3],float L[3],float left[3],float up[3],float front[3],size_t &localFileOffset);
int read_mhd_data(string fname,string &dtype,int nn[3],size_t &localFileOffset,char **data);

template<typename T>
int read_mhd(string fname,vector<T> &A,int nn[3],float x0[3],float L[3],float left[3],float up[3],float front[3])
{
	string dtype_src;
	size_t localFileOffset;
	int ierr = read_mhd_header(fname,dtype_src,nn,x0,L,left,up,front,localFileOffset);
	if(ierr) return ierr;
	...
	fin.seekg(localFileOffset,fin.beg);
