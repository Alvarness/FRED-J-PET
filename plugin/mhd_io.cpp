...
int read_mhd_header(string fname,string &dtype,int nn[3],float x0[3],float L[3],float left[3],float up[3],float front[3],size_t &localFileOffset)
{
    ...
    while(1){
        ...
        if(icompare(tok,"ElementDataFile")==0){
            ...
            if(tok!="=") {cerr<<"Error in reading mhd header"<<endl; return 1;}
            fin>>tok;
            if(icompare(tok,"LOCAL")){cerr<<"Error: expected ElementDataFile = LOCAL"<<endl;return 1;}
            if(fin.peek()=='\r') fin.seekg(1,fin.cur);
            if(fin.peek()=='\n') fin.seekg(1,fin.cur);
            localFileOffset = fin.tellg();
            break;
        }
        // cout<<tok<<endl;
        // if(tok.rfind("ElementDataFile")!=string::npos) break;
    }
    if(debug) cout<<"local file offset = "<<localFileOffset<<endl;
    fin.close();
...
int read_mhd_data(string fname,string &dtype,int nn[3],size_t &localFileOffset,char **data)
{
    ...
    cout<<"localFileOffset "<<localFileOffset<<endl;
